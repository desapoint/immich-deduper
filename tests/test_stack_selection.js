...
async function main() {
	...
	assert.equal(ste.dispatchAction({id: JSON.stringify({type: 'btn-view-group', id: 'asset-2'})}, 'sim-view-group-trigger'), true)
	assert.equal(actionSyncs.at(-1).id, 'sim-view-group-trigger')
	assert.equal(actionSyncs.at(-1).data.id.id, 'asset-2')
