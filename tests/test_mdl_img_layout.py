...
class TestImageViewerLayout(unittest.TestCase):
	def test_viewer_exposes_clear_controls_and_accessible_navigation(self):
		...
		self.assertTrue(any(props(node).get('children') == 'Shortcuts' and props(node).get('className') == 'visually-hidden' for node in walk(byId[mdlImg.k.btnHelp])))
		self.assertTrue(any(props(node).get('children') == 'Details' and props(node).get('className') == 'visually-hidden' for node in walk(byId[mdlImg.k.btnInfo])))
