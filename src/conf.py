...
class co:
	class to:
		@classmethod
		def dict(cls): return {key: value for key, value in vars(cls).items() if not key.startswith('_') and not callable(value)}
	...
	class fmt:
		...
		def size(value):
			if isinstance(value, (int, float)):
				if value > 1024 * 1024: return f"{value / (1024 * 1024):.2f} MB"
				elif value > 1024: return f"{value / 1024:.2f} KB"
...
class url:
	@staticmethod
	def get_image_url(assetId, photoQ=ks.db.thumbnail): return f"/api/image/{assetId}?quality={photoQ}"
...
class envs:
	...
	if os.getcwd().startswith(os.path.join(pathRoot, 'tests')): ddupData = os.path.join(pathRoot, 'data/')
	else:
	...
	def showVars():
		def maskSensitive(value: str, keepStart: int=2, keepEnd: int=1) -> str:
			if not value or len(value) <= keepStart + keepEnd: return '*' * len(value) if value else ''
		...
		if envs.isDock:
			...
			if envs.envImmichThumb: lg.info(f"  IMMICH_THUMB: {envs.envImmichThumb} → {envs.immichThumb}")
			else: lg.info(f"  IMMICH_THUMB: (not set)")
		else:
...
def getEnvs(): return {'port':envs.ddupPort}
